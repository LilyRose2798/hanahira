...
export const bmvbhashEven = (data: { width: number, height: number, data: Buffer | Uint8ClampedArray }, bits: number) => {
...
export const bmvbhash = (data: { width: number, height: number, data: Buffer | Uint8ClampedArray }, bits: number) => {
  const result: number[] = []
  let weightTop
  let weightBottom
  let weightLeft
