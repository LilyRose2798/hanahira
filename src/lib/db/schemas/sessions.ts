...
export const sessionSchema = zodModel<_Session>("session")({
  ...
  expiresAt: z.coerce.date().openapi({ description: "The date the session expires at", example: new Date(0) }),
  createdAt: z.coerce.date().openapi({ description: "The date the session was created at", example: new Date(0) }),
})
