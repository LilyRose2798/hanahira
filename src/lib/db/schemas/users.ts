...
export const totpSchema = z.string().regex(/^\d*$/, "Code must contain only numerical digits")
  .length(6, "Code must be exactly 6 digits long")
  .openapi({ description: "The TOTP code", example: "294817" })
export const passwordSchema = z.string().min(1).max(256)
  .openapi({ description: "The user's password", example: "hunter2" })
export const signInSchema = z.object({ username: userSchema.shape.username, password: passwordSchema, totp: totpSchema.optional() })
